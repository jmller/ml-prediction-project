...
# -*- coding: utf-8 -*-
"""
This is the main-program containing the inner and outer loop
...
###
def all_params_loop(final_table):
    count = 0    
    start_iteration = 0
...
def main_loop(config_list, final_table, count): 
    ...
    for x in range(tune_size+10, len(X)-1):
        ...
        ###
        X_train, X_test, y_train, y_test = ds.data_split(X, y, x, timeseries, config_list)
        
        ###
        # Actual Training and Prediction
        ###
        if config_list['chosen_method'] == config.method [0]:
            y_pred, opt_modulo_params = ml_nb.classifier(X_train, y_train, X_test, x, tune_size, config_list, opt_modulo_params)
        elif config_list['chosen_method'] == config.method [1]:
            y_pred, opt_modulo_params = ml_rf.classifier(X_train, y_train, X_test, x, tune_size, config_list, opt_modulo_params)
        elif config_list['chosen_method'] == config.method [2]:
            y_pred, opt_modulo_params = ml_svc.classifier(X_train, y_train, X_test, x, tune_size, config_list, opt_modulo_params)         
        elif config_list['chosen_method'] == config.method [3]:
            y_pred, opt_modulo_params = ml_ann.classifier(X_train, y_train, X_test, x, tune_size, config_list, opt_modulo_params)
        else:
        ...
        if opt_modulo_params != 0 and x%200 == 0:       
            ...
            opt_modulo_params_w_day['chosen_method'] = config_list['chosen_method']
        ...
        current_measures = pd.DataFrame({'day': [x], 'accuracy': [accuracy], 'f-measure': [f_measures]})
    ...
    if config.documentation == 1:
        ...
        documentation.save_doc(config_list, online_measures, dataset_input, df, count, opt_modulo_params_list)
